...
stuInfo.each do |st|
  if (Date.today - DateTime.parse(st[:deadline].to_s).to_date) <= 0
    deactivation.push(:id => st[:id], :sbmtime => st[:sbmtime].to_s, :deadlineDay => DateTime.parse(st[:deadline].to_s).to_date)
  elsif (Date.today - DateTime.parse(st[:reminder].to_s).to_date) <= 0
    reminder.push(:id => st[:id], :sbmtime => st[:sbmtime].to_s, :reminderDay => DateTime.parse(st[:reminder].to_s).to_date)
  end
end
